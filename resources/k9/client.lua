...
--[[ Main Event Handlers ]]--
    ...
    AddEventHandler("K9:OpenMenu", function(pedRestriction, pedList)
        if pedRestriction then
            if CheckPedRestriction(GetLocalPed(), pedList) then
                ...
                Notification(tostring("~r~You do not have the right PED to use the pet."))
            end
        else
            EnableMenu()
        end
    ...
    AddEventHandler("K9:ToggleK9", function(model)
        if spawned_ped == nil then
            ...
            local dog = CreatePed(28, ped, plyCoords.x, plyCoords.y, plyCoords.z, GetEntityHeading(GetLocalPed()), true, false)
            spawned_ped = dog
            ...
            AddTextComponentString(tostring("Pet: ".. k9_name))
...
--[[ Threads ]]
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            -- Trigger Attack
            if IsControlJustPressed(1, 166) and IsPlayerFreeAiming(PlayerId()) then
