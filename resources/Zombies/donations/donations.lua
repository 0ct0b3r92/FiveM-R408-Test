...
RegisterCommand("giveloadout", function(source,args,raw)
	...
	elseif loadoutCount > 0 then
		...
		GiveWeaponToPed(PlayerPedId(), GetHashKey("WEAPON_PISTOL"), 30, false, false)
		GiveWeaponToPed(PlayerPedId(), GetHashKey("WEAPON_APPISTOL"), 90, false, false)
		GiveWeaponToPed(PlayerPedId(), GetHashKey("WEAPON_COMBATPDW"), 60, false, false)
...
-- Allows to give inventory loadout once per respawn
RegisterCommand("giveinventory", function(source,args,raw)
	if donated == nil then
		TriggerEvent("chatMessage", "^1SYSTEM", {255, 255, 255}, "You are not allowed to use this command.")
	elseif itemCount > 0 then
		...
		-- Code will go here once DB is done--
		TriggerEvent("chatMessage", "^2SYSTEM", {255, 255, 255}, "You were given the donators inventory loadout which includes some crafting materials and items.")
	end
end)
