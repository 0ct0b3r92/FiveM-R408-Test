...
RegisterCommand("randomdrop", function(source,args,raw)
	dropCount = dropCount + 1
	if dropCount <= 1 then
		TriggerEvent("Randomdrop:Execute")
		TriggerServerEvent("SupplyInbound")
	elseif dropCount > 1 then
		TriggerEvent("chatMessage", "^2MERRYWEATHER", {255, 255, 255}, "There are no supply drops at this time.")
	end
end, false)

RegisterNetEvent("Randomdrop:Execute")

AddEventHandler("Randomdrop:Execute", function(weapon, ammo)
    Citizen.CreateThread(function()
        ...
        TaskVehicleDriveToCoord(pilot, aircraft, fx, fy, fz + 200, 60.0, 0, GetHashKey("bombushka"), 262144, 15.0, -1.0);
		...
		-- This determines how high above the ground or players position to start the drop
		dropHeight = 250
		
        TaskVehicleDriveToCoord(pilot, aircraft, 0, 0, 500, 60.0, 0, GetHashKey("bombushka"), 262144, -1.0, -1.0)
		...
		randomLoot = weaponList[math.random(1, #weaponList)]
		
        weaponInsideCrate = CreateAmbientPickup(GetHashKey(randomLoot), randomX, randomY, cz + dropHeight, 0, ammo, GetHashKey("ex_prop_adv_case_sm"), true, true)
        ...
        --StartParticleFxLoopedOnEntity_2("scr_crate_drop_flare", weaponInsideCrate, 0.0, 0.0, 0.2, 0.0, 0.0, 0.0, 1.0, false, false, false) -- no idea how to make it work, weapon_flare will do for now
        StartParticleFxLoopedOnEntity("scr_crate_drop_smoke", weaponInsideCrate, 0.0, 0.0, 0.2, 0.0, 0.0, 0.0, 1.0, true, true) -- no idea how to make it work, weapon_flare will do for now
        ...
        ShootSingleBulletBetweenCoords(jx, jy, jz, jx, jy + 0.0001, jz - 0.0001, 0, false, GetHashKey("weapon_flare"), -1, true, false, -1.0)
